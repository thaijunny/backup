...
--            v_where := v_where || ' AND UPPER(a.name) LIKE ''%' || UPPER(TRIM(pv_name)) || '%''';
    ...
    PROCEDURE search_branch (
        ...
        pv_size         IN NUMBER)
    AS
        -- ThaiDT FINY-83: Update query support optional district (change INNER JOIN to LEFT JOIN for district) 29/12/2025
...
--        INTO v_aid  FROM area;
    ...
    BEGIN
        ...
        IF v_counterror > 0 THEN
            ...
            RETURN;
        END IF;
      -- ThaiDT FINY-83: Update address support optional district (ward -> district -> province OR ward -> province)  29/12/2025
        --  map id địa chỉ
        IF pv_district_id IS NOT NULL THEN
            ...
            select w.wlevel ||' '|| w.wname || ', '|| d.dlevel ||' '|| d.dname || ', '|| p.pname into v_full_address from ward w, district d, province p
                ...
                and d.pid=p.id;
        ELSE
            -- ThaiDT FINY-83: Model không có huyện: xã -> tỉnh  29/12/2025
            ...
            IF v_counterror = 0 THEN
                ...
                RETURN;
            END IF;
            -- ThaiDT FINY-83: join địa chỉ không có huyện  29/12/2025
